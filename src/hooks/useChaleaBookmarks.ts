...
export function useChaleaBookmarks() {
  const { user } = useAuth()
  ...
  const toggleBookmark = async (skillId: number) => {
    return isBookmarked(skillId) ? removeBookmark(skillId) : addBookmark(skillId)
  }

  return { bookmarks, loading, addBookmark, removeBookmark, isBookmarked, toggleBookmark, refetch: fetchBookmarks }
}
